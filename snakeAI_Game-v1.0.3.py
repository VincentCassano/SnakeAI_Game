...
class SnakeGame:
    def __init__(self, seed=0, board_size=50, silent_mode=False):
        ...
        # 显示窗口尺寸
        self.display_width = self.width + 2 * self.border_size + 240   # 包含侧边栏
        self.display_height = self.height + 2 * self.border_size + 40  # 包含顶部信息栏
    ...
    def draw_opponent_snake(self):
        ...
        if self.opponent_direction == "UP":
            ...
            eye_y_offset = 0  # 初始化eye_y_offset以避免错误
        else:  # RIGHT
            eye_x_offset1 = eye_offset
            eye_x_offset2 = eye_offset
            eye_y_offset1 = -eye_offset
        ...
        # 绘制眼睛（白色基础+黑色瞳孔）
        if self.opponent_direction in ["LEFT", "RIGHT"]:
            # 左眼
            pygame.draw.circle(self.screen, (255, 255, 255), 
                              ...
                              eye_size)
            # 左眼瞳孔（朝移动方向看）
            pupil_offset = eye_size // 2 * (1 if self.opponent_direction == "RIGHT" else -1)
            pygame.draw.circle(self.screen, (0, 0, 0), 
                              (head_x + self.cell_size // 2 + eye_x_offset1 + pupil_offset, 
                               head_y + self.cell_size // 2 + eye_y_offset1), 
                              eye_size // 2)
            # 右眼
            pygame.draw.circle(self.screen, (255, 255, 255), 
            ...
            # 右眼瞳孔
            pygame.draw.circle(self.screen, (0, 0, 0), 
                              (head_x + self.cell_size // 2 + eye_x_offset2 + pupil_offset, 
                               head_y + self.cell_size // 2 + eye_y_offset2), 
                              eye_size // 2)
        else:  # UP, DOWN
            # 左眼
            pygame.draw.circle(self.screen, (255, 255, 255), 
                              ...
                              eye_size)
            # 左眼瞳孔
            pupil_offset = eye_size // 2 * (1 if self.opponent_direction == "DOWN" else -1)
            pygame.draw.circle(self.screen, (0, 0, 0), 
                              (head_x + self.cell_size // 2 + eye_x_offset1, 
                               head_y + self.cell_size // 2 + eye_y_offset + pupil_offset), 
                              eye_size // 2)
            # 右眼
            pygame.draw.circle(self.screen, (255, 255, 255), 
                              (head_x + self.cell_size // 2 + eye_x_offset2, 
        ...
        for i, (r, c) in enumerate(self.opponent_snake[1:]):
            ...
            total_segments = len(self.opponent_snake) - 1
            if total_segments > 0:
                progress = 1 - (i / total_segments)
                # 使用立方函数让颜色变化更自然
                progress_cubed = progress ** 3
                
                # 红色渐变：从头部附近的亮红色到尾部的暗红色
                red_intensity = int(100 + 80 * progress_cubed)
                green_blue = int(20 + 30 * progress_cubed)
                
                # 添加一点随机性使身体更有趣
                random_offset = random.randint(-5, 5)
                segment_color = (max(30, min(200, red_intensity + random_offset)), 
                                max(0, min(50, green_blue + random_offset)), 
                                max(0, min(50, green_blue + random_offset)))
            ...
            if i == total_segments - 1 and total_segments > 1:
                prev_r, prev_c = self.opponent_snake[-2]
                # 确定尾部方向
                if prev_r < r:  # 尾部向下
        ...
        body_length = len(self.opponent_snake) - 1
        for i, (r, c) in enumerate(self.opponent_snake[1:]):
            body_x = c * self.cell_size + self.border_size
            body_y = r * self.cell_size + self.border_size
            
            # 计算渐变颜色 - 从头部附近的亮红色逐渐过渡到尾部的深红色
            progress = i / max(body_length, 1)
            # 使用平方函数使渐变更自然（开始变化慢，后面变化快）
            red_intensity = int(255 - (100 * progress * progress))
            
            # 根据不同部分使用微妙的颜色变化
            if i == 0:  # 靠近头部的第一节
                color = (red_intensity, 50, 50)
            else:
                color = (red_intensity, 20, 20)
...
def main_gui():
    ...
    buttons = [
        ...
        # 第二行按钮
        Button(center_x - btn_width//2, second_row_y, btn_width, btn_height, 
